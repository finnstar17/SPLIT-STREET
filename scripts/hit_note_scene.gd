...
var bounce_note = preload("res://materials/note_type_stuff/bounce_note.tres")
...
func _set_position():
	...
	if type == 2:
		...
		climb_mesh.show()
	if type == 3:
		note_mesh.set_surface_override_material(0, bounce_note)

		var bounce_mesh = $BounceNoteMesh
		bounce_mesh.show()
